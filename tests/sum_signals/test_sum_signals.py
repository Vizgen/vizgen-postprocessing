...
class SumSignalsCaseScheme:
    ...
    def __init__(self, name: str, images: list, boundaries: gpd.GeoDataFrame,
                 micron_to_mosaic: np.ndarray, answer: pd.DataFrame, workers: int,
                 temp_dir: TempDir):
        self.name = name
...
class SumSignalsCase(BaseCase):
    ...
    def __init__(self, name: str, regex: str, boundaries_path: str,
                 transform_path: str, output_path: str, answer: pd.DataFrame, workers: int):
        super().__init__(name)
...
SUM_SIGNALS_TEST_SCHEMES = [
    SumSignalsCaseScheme('z_layers', [(np.ones((100, 100)), 'c1', 1),
...
def setup_from_scheme(scheme: SumSignalsCaseScheme) -> BaseCase:
    ...
    with vzg_open(boundaries_path, 'wb') as f:
        scheme.boundaries.to_parquet(f)

    transform_path = sep.join([path, 'transform.csv'])
    with vzg_open(transform_path, 'wb') as f:
        np.savetxt(f, scheme.micron_to_mosaic)

    output_path = sep.join([path, 'output.csv'])

    yield SumSignalsCase(scheme.name, regex, boundaries_path,
                         transform_path, output_path, scheme.answer, scheme.workers)
...
def test_sum_signals(scheme: SumSignalsCaseScheme, setup_from_scheme: SumSignalsCase):
    args = argparse.Namespace(input_images=setup_from_scheme.regex,
    ...
    with vzg_open(setup_from_scheme.output_path, 'r') as f:
        output_df = pd.read_csv(f, index_col=0)
