...
class GetCellBrightnessInImageCase(BaseCase):
    ...
    def __init__(self, name: str, image: np.ndarray, boundaries: List, sum_signal: np.ndarray):
...
def test_1d_poly_does_not_crash():
    ...
    boundaries = [
        MultiPolygon([([(128, 192), (256, 192), (256, 192), (128, 192)], [])]),
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        get_cell_brightness_in_image(str(img_path), ((row["EntityID"], row["Geometry"]) for _, row in gdf.iterrows()))
