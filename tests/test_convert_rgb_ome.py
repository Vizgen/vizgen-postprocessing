...
def test_rgb_ome(test_dir: TempDir, setup_rgb_ome: Namespace):
    ...
    with get_rasterio_environment(setup_rgb_ome.output_image):
        with rasterio_open(setup_rgb_ome.input_image_red) as file_r, \
             rasterio_open(setup_rgb_ome.input_image_green) as file_g, \
             rasterio_open(setup_rgb_ome.input_image_blue) as file_b:
